...
# Page configuration
st.set_page_config(
    page_title="AI Co-Creation - High Recommendation Level",
    page_icon="👕",
    layout="wide",
...
if 'experiment_group' not in st.session_state:
    st.session_state.experiment_group = "High Recommendation Level"
...
def generate_multiple_designs(design_prompt, count=5):
    """生成多个设计变体"""
    designs = []
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=count) as executor:
        # 创建任务
        future_to_id = {
            executor.submit(generate_single_design, design_prompt, f"var_{i}"): i 
        ...
        for future in concurrent.futures.as_completed(future_to_id):
            ...
            try:
                ...
                if design:
                    ...
                    print(f"Failed to generate design {design_id}: {info}")
            except Exception as e:
                print(f"Exception for design {design_id}: {str(e)}")
...
def main():
    ...
    with input_col:
        ...
        if st.session_state.should_generate:
            with st.spinner("AI正在为您生成多种T恤设计选项..."):
                try:
                    st.session_state.is_generating = True
                    
                    # 进度条，因为高级版本生成5个设计需要更长时间
                    ...
                    progress_future = progress_thread.submit(update_progress)
                    
                    # 生成5个设计选项
                    designs = generate_multiple_designs(st.session_state.user_prompt, count=5)
